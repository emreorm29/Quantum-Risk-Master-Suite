...
# --- 1. SİSTEM VE VERİTABANI KURULUMU ---
def init_db():
...
# --- 2. HASSAS GİRDİ BİLEŞENİ (Slider + Sync Input) ---
...
class RiskEngine:
    ...
    def draw_credit_heatmap(ead):
        ...
        plt.xlabel("Temerrüt Olasılığı (PD)")
        plt.ylabel("Kayıp Oranı (LGD)")
        return fig
...
module = st.sidebar.selectbox("Modüller", 
    ["🛡️ Aktüeryal Tazminat", "💰 Yatırım & NPV", "🏦 Kredi Riski (Basel III)", "📜 Arşiv"])

# Canlı Veri Butonu
if st.sidebar.button("🌍 Canlı Piyasa Güncelle"):
    try:
        tnx = yf.Ticker("^TNX").history(period="1d")['Close'].iloc[-1]
        st.session_state['live_rate'] = tnx
        notify("Piyasa Güncellendi", f"ABD 10Y: %{tnx:.2f}")
    except: st.sidebar.error("Veri çekilemedi.")

# --- MODÜL İŞLEMLERİ ---

if module == "🛡️ Aktüeryal Tazminat":
    st.title("🛡️ Aktüeryal Tazminat Terminali")
    c1, c2 = st.columns(2)
    with c1:
        name = st.text_input("Maktul/Dosya Adı", "Ahmet Yılmaz")
    ...
    with c2:
        ...
        if st.button("Kaydet"):
            save_to_db(name, "Aktüeryal", total, f"Yaş: {age}, Faiz: %{rate}")
            st.success("Arşive eklendi.")

elif module == "💰 Yatırım & NPV":
    st.title("💰 Yatırım Analizi (NPV)")
    c1, c2 = st.columns(2)
    with c1:
        p_name = st.text_input("Proje", "Girişim X")
        base = st.number_input("Başlangıç Nakit Akışı", value=1000000)
        growth = precise_input("Yıllık Büyüme (%)", 0.0, 100.0, 10.0, 0.1, "growth")
        disc = precise_input("İskonto Oranı (%)", 0.0, 100.0, 15.0, 0.1, "disc")
    ...
    with c2:
        ...
        st.metric("Net Bugünkü Değer", f"{npv:,.2f} TL")
        st.area_chart(cfs)

elif module == "🏦 Kredi Riski (Basel III)":
    st.title("🏦 Kredi Riski ve Isı Haritası")
    c1, c2 = st.columns(2)
    with c1:
        client = st.text_input("Müşteri", "A.Ş. Holding")
        ead = st.number_input("Risk Tutarı (EAD)", value=5000000)
        pd = precise_input("Temerrüt Olasılığı (PD %)", 0.1, 20.0, 5.0, 0.05, "pd")
        lgd = precise_input("Kayıp Oranı (LGD %)", 10.0, 100.0, 45.0, 0.1, "lgd")
    with c2:
        el = (pd/100) * (lgd/100) * ead
        st.metric("Beklenen Kayıp (EL)", f"{el:,.2f} TL")
        st.metric("Sermaye Yükü (RWA)", f"{el * 12.5:,.2f} TL")
        summary = f"{client} için EL: {el:,.2f} TL. Risk düzeyi PD:%{pd} ile analiz edildi."
        st.info(f"**Yönetici Özeti:** {summary}")

    st.divider()
    st.subheader("🔥 Dinamik Risk Isı Haritası")
    st.pyplot(RiskEngine.draw_credit_heatmap(ead))
    
    if st.button("Risk Analizini Kaydet"):
        save_to_db(client, "Kredi Riski", el, summary)
        notify("Kayıt Başarılı", f"{client} arşive eklendi.")

elif module == "📜 Arşiv":
    st.title("📜 Risk Analiz Arşivi")
    conn = sqlite3.connect('quantum_ultimate.db')
    df = pd.read_sql_query("SELECT * FROM risk_logs ORDER BY date DESC", conn)
    st.dataframe(df, use_container_width=True)
    if st.button("Arşivi Sıfırla"):

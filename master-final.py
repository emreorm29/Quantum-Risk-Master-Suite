...
# --- 1. SİSTEM VE VERİTABANI FONKSİYONLARI ---
def init_db():
...
def get_live_usd():
    try:
        ...
        return 35.85 # Hata durumunda varsayılan kur
...
def generate_ultimate_pdf(title, category, value, summary, scenario_df=None, plot_fig=None):
    ...
    if scenario_df is not None:
        ...
        for _, row in scenario_df.iterrows():
            pdf.cell(80, 10, f"{clean(row['Senaryo'])}: {row['Kayıp']:,.2f} TL", 1, 1)
...
if not st.session_state.auth:
    st.title("💎 Quantum Risk Master Suite")
    st.info("Bu terminal Arch Linux ortamında yüksek güvenlikli aktüeryal hesaplamalar için tasarlanmıştır.")
    st.warning("Lütfen sol panelden kimlik doğrulaması yapınız.")
    login_screen()
else:
    # --- CANLI VERİ PANELİ ---
    usd_val = get_live_usd()
    st.sidebar.success(f"Oturum Açıldı: Admin")
    st.sidebar.metric("Canlı USD/TRY", f"{usd_val:.4f}")
    if st.sidebar.button("Çıkış Yap"):
        st.session_state.auth = False
        st.rerun()

    module = st.sidebar.selectbox("Yönetim Paneli", ["📊 Kokpit", "🛡️ Aktüeryal", "💰 Yatırım & Monte Carlo", "🏦 Kredi Riski", "📜 Arşiv"])

    # --- MODÜL: KOKPİT ---
    if module == "📊 Kokpit":
        st.title("📊 Yönetici Risk Kokpiti")
        conn = sqlite3.connect('quantum_ultimate.db')
        ...
        if not df.empty:
            ...
            c3.metric("Döviz Karşılığı ($)", f"${(df['value'].sum() / usd_val):,.0f}")
            st.divider()
            fig_bar, ax_bar = plt.subplots(figsize=(10, 4))
            sns.barplot(data=df, x='category', y='value', ax=ax_bar, palette="viridis")
            st.pyplot(fig_bar)
        else:
            st.info("Sistemde henüz kayıtlı analiz bulunmamaktadır.")

    # --- MODÜL: AKTÜERYAL ---
    elif module == "🛡️ Aktüeryal":
        st.title("🛡️ Aktüeryal Tazminat")
        c1, c2 = st.columns(2)
        with c1:
            name = st.text_input("Vaka Adı", "Tazminat Dosya-01")
            age = st.number_input("Mevcut Yaş", 0, 90, 35)
            income = st.number_input("Aylık Gelir (TL)", value=45000)
        ...
        with c2:
            ...
            res = ann * income * 12
            st.metric("Hesaplanan Yükümlülük", f"{res:,.2f} TL")
    ...
    # --- MODÜL: YATIRIM & MONTE CARLO ---
    elif module == "💰 Yatırım & Monte Carlo":
        st.title("💰 Yatırım & Monte Carlo Risk Dağılımı")
        c1, c2 = st.columns(2)
        with c1:
            p_name = st.text_input("Proje Adı", "Stratejik Yatırım")
            mu = st.number_input("Beklenen Yıllık Getiri (TL)", value=500000)
            vol = st.slider("Olası Sapma (Volatilite %)", 5, 50, 20)
        with c2:
    ...
    # --- MODÜL: KREDİ RİSKİ ---
    elif module == "🏦 Kredi Riski":
        st.title("🏦 Kredi Riski & Stres Testi")
        c1, c2 = st.columns(2)
        with c1:
            cl = st.text_input("Müşteri", "Kurumsal Portföy")
            ead = st.number_input("EAD (Risk Tutarı)", value=1000000)
            pd_v = st.slider("PD (Olasılık %)", 0.1, 20.0, 5.0)
            lgd_v = st.slider("LGD (Kayıp %)", 10.0, 100.0, 45.0)
        ...
        for sn, m in scs.items():
            ...
            s_data.append({"Senaryo": sn, "Kayıp": loss})
        df_s = pd.DataFrame(s_data)
        
        ct, cg = st.columns(2)
        ct.table(df_s)
        fig_s, ax_s = plt.subplots(figsize=(6,3))
        ax_s.bar(df_s["Senaryo"], df_s["Kayıp"], color=['green', 'blue', 'red'])
        cg.pyplot(fig_s)
        
        if st.button("Stres Testi Raporu Üret"):
            ...
            st.download_button("📥 PDF Rapor", data=pdf, file_name=f"{cl}_Risk.pdf")
